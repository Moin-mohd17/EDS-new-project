...
// document.querySelector('header').insertAdjacentElement('afterbegin', overlay);
function toggleAllNavSections(sections, expanded = false) {
  sections.querySelectorAll('.header-new >div >div > ul > li').forEach((section) => {
    section.setAttribute('aria-expanded', expanded);
  });
}
export default async function decorate(block) {
  ...
  console.log(navContent, "Cleaned fragment");
  ...
  if (main) {
    main.querySelectorAll('.section').forEach((section) => {
      ...
      console.log(wrapper, ':broom: Removing duplicate nav section from <main>:', newWrapper);
      wrapper?.remove();
      newWrapper?.remove();
    });
  }
  const navSections = document.querySelector('.header-new >div >div>ul');
  toggleAllNavSections(navSections);

  if (navSections) {
    navSections.querySelectorAll('li').forEach(async (navSection) => {
  ...
  // const modalFragment = await loadFragment(modalPath);
  // document.body.append(modalFragment);

  const navSectionss = document.querySelector('.default-content-wrapper');
  console.log("fnauinfdas", navSections);

  // toggleAllNavSections(navSections);
  if (navSectionss) {
    navSectionss.querySelectorAll(':scope > ul > li').forEach((navSection, index) => {
      // if (navSection.querySelector('ul')) navSection.classList.add('nav-drop');
      navSection.addEventListener('click', async () => {
        // if (index === 0) {
        //   const hoverContainerPath = '/modals/location-modal';
        //   const fragment = await loadFragment(hoverContainerPath);
        //   await showModal(fragment);
        // }
        if (index === 1) {
      ...
      navSection.addEventListener('mouseleave', () => {
        // if (isDesktop.matches) {
        // const expanded = navSection.getAttribute('aria-expanded') === 'true';
        toggleAllNavSections(navSectionss);
        navSection.setAttribute('aria-expanded', 'false');
        // }
      });
    });
    ...
    if (locationLi) {
      ...
      const hoverContainerPath = '/modal/location';
      const fragment = await loadFragment(hoverContainerPath);
      // locationLi.addEventListener('mouseenter', async () => {
      //   console.log("???????????????????????");
      //   showModal(fragment);
      // });
      locationLi.addEventListener('click', async () => {
        ...
        `;
          ...
          applyButton.addEventListener('click', async e => {
            ...
            // console.log(JSON.stringify(result, null, 5), '📍 Valid Pincode Submitted:');
            document.querySelector('dialog')?.close(); // optional: close modal
          });
        }, 1000);
      });
    ...
    const topLevelItems = document.querySelectorAll('.header-new > div > div > ul > li');
